...
> The form contents are **not** stored on the server. If no-one has the form
> open in a browser, the contents are gone.
...
1. The form data could be persisted, even during editing, in browser persistent
   storage, or also by having another clone on the server (not just a passive
   message passer). This would mean it's possible to look at an existing form
